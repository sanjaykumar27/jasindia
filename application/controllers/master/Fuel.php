...
class Fuel extends CI_Controller {
    ...
    public function index()
    {
        ...
        {
            $this->load->view('Master/fuel_master');
        } else
        {
            redirect('/Auth');
        }
            ...
            if (!empty($data['records']))
            {
                foreach ($data['records'] as $value)
                {
                    $html .= '<tr><td>'.$i.'</td><td class="text-truncate">' . $value->fuel_name . '</td><td>'
                            . '<a href="javascript:void(0)" id="m_editbutton" data-toggle="modal" value="'.$value->fuel_id.'"  data-target="#ModalUpdateFuel" class="btn m-btn--pill btn-outline-success btn-sm"><i class="fa fa-pencil-alt"></i> Edit</a></td>';
                $i++; }
            }
